...
public class Topology extends _Properties implements ClockListener{
    ...
    //create for storing all the labeledLinkeds in Discrete topoloty
    public List<LabeledLink> labeledLinks = new ArrayList<LabeledLink>();
    //create for storing all the robots in topology
    public List<Robot> robots = new ArrayList<Robot>();
    //create for storing all the obstacles in topology
    public List<Obstacle> obstacles = new ArrayList<Obstacle>();
    //create for storing all the robots which are moved in next round
    public List<Robot> robotsToMove = new ArrayList<Robot>();
    ...
    */
    ...
    /**
     * Create by JiaqiXu, used for adding robots and collect all robots in this topology
    ...
    /**
     * Create by JiaqiXu, used for adding obstacle and collect all obstacles in this topology
    ...
    //created for add LabeledLink
    public void addLabeledLink(LabeledLink l)
    {
        labeledLinks.add(l);   
    	addLink(l, false);
    }
    ...
    /**
     * 
     * @param number represents the number of robots that are in crash faulty
     */
    public void setCrashFaulty(int number)
    {
    	if(number > robots.size())
    	{ 
    		System.out.println("the number of faulty robots should less or equal"
    				+ " than the number of total robots in this topology");
    		return;
    	}
    	
    	//get @number different numbers between 0 and robots.size()-1
       int[] indexOfFaultyRobots = new int[number];
       for(int i =0; i<number;i++)
       {
    	   indexOfFaultyRobots[i]=-1;
       }
       int count = 0;
       boolean flag;
       while(count < number)
       {
    	   int index = (int)(Math.random()*robots.size());
    	   flag = true;
    	   for(int i =0; i<number; i++)
    	   {
    		   if(index ==indexOfFaultyRobots[i])
    		   {
    			   flag =false;
    			   break;
    		   }
    	   ...
    	   if(flag)
    	   {
    		   indexOfFaultyRobots[count]= index;
    		   count ++;
    	   }   	   
       }
       
       //for each chosen robot, assign the faulty round.
       Robot faultyRobot;
       for(int i=0; i<number; i++)
       {
    	  faultyRobot = robots.get(indexOfFaultyRobots[i]);
    	  faultyRobot.faultyRound = (int)(Math.random()*100); // faultRound between 0 - 99
       }
     
       if(this.nodeScheduler instanceof DefaultRobotScheduler){   	   

    	   ((DefaultRobotScheduler)this.nodeScheduler).addPostComputeListener(new ClockListener() {
		@Override
		public void onClock() {
			
			int clocknumber=clock.time;
			System.out.println("This is round "+clocknumber );
			for(int j: indexOfFaultyRobots){
			    Robot faulty=robots.get(j);
				if(clocknumber>=faulty.faultyRound){
					System.out.println("robot "+ faulty.name + " in crash faulty in round "+clocknumber);
					if(faulty instanceof RobotDiscrete)
					{
						((DestinationDiscrete)faulty.dest).outport = -1; //for robotsDiscrete, -1 means stay on site
					}
					else{
						((Destination2D)(faulty.dest)).dest = faulty.getLocation(); 
					}
				}				
			}
		}	
    ...
    public  void setByzantineFaulty(int number)
    {
    	if(number > robots.size())
    	{ 
    		System.out.println("the number of faulty robots should less or equal"
       ...
       //for each chosen faulty robot, it will choose one of these three behavior in each round.
       //0 represents stay on site; 1 represents move correctly; 2 represents move to a destination
       // which is not the computed destination
       ...
       if(this.nodeScheduler instanceof DefaultRobotScheduler){   	   
           ((DefaultRobotScheduler)this.nodeScheduler).addPostComputeListener(new ClockListener() {
    		...
    		public void onClock() {
    			int clocknumber = clock.time;
    			System.out.println("This is round "+clocknumber );
    	          for(int i: indexOfFaultyRobots)
    	          {
    	        	Robot faultyRobot = robots.get(i);
    	        	int  indexOfBehavior = (int)(Math.random()*3);
    	        	  switch (indexOfBehavior) {
					case 0:
						((Destination2D)(faultyRobot.dest)).dest = faultyRobot.getLocation(); 
						System.out.println("robot " + faultyRobot.name + " stay put in round " + clocknumber);
						break;
					case 1:
						//do nothing, everything is normal. i.e., robot will move to computed destination
						System.out.println("robot " + faultyRobot.name + " move correctly in round " + clocknumber);
						break;
					case 2:
						double faultyDest_x = ((Destination2D)(faultyRobot.dest)).dest.getX() + Math.random()*20 -10;
						double faultyDest_y = ((Destination2D)(faultyRobot.dest)).dest.getY() + Math.random()*20 -10;
						((Destination2D)(faultyRobot.dest)).dest = new Point2D.Double(faultyDest_x,faultyDest_y);
						System.out.println("robot " + faultyRobot.name + " move to a unpredicted position in round " + clocknumber);
					default:
						break;
					}
    	          } 
    ...
    public void setSelfStabilization(int number)
    {
    	if(number > robots.size())
    	{ 
    		System.out.println("the number of faulty robots should less or equal"
       ...
       //for each chosen robot, assign the faulty round from which the faulty robot become normal
       Robot faultyRobot;
       for(int i=0; i<number; i++)
       {
    	  faultyRobot = robots.get(indexOfFaultyRobots[i]);
    	  faultyRobot.faultyRound = (int)(Math.random()*100); 
       }
       
       //if current round < faultyround, the chosen robots's behavior could be one of these three behavior;
       //otherwise, the chosen robots work normally.
       //0 represents stay on site; 1 represents move correctly; 2 represents move to a destination
       //which is not the computed destination
				...
				if(clocknumber < faulty.faultyRound){
				 int indexOfBehavior = (int)(Math.random()*3);
				  switch (indexOfBehavior) {
					case 0:
						((Destination2D)(faulty.dest)).dest = faulty.getLocation(); 
						System.out.println("robot " + faulty.name + " stay put in round " + clocknumber);
						break;
					case 1:
						//do nothing, everything is normal. i.e., robot will move to computed destination
						System.out.println("robot " + faulty.name + " move correctly in round " + clocknumber);
						break;
					case 2:
						double faultyDest_x = ((Destination2D)(faulty.dest)).dest.getX() + Math.random()*20 -10;
						double faultyDest_y = ((Destination2D)(faulty.dest)).dest.getY() + Math.random()*20 -10;
						((Destination2D)(faulty.dest)).dest = new Point2D.Double(faultyDest_x,faultyDest_y);
						System.out.println("robot " + faulty.name + " move to a unpredicted position in round " + clocknumber);
					default:
						break;
				}		
			  }				
    ...
    public void setInaccuracies(int number, double limited_angle, double limited_distance)
    {
    	if(number > robots.size())
    	{ 
    		System.out.println("the number of inaccuracy robots should less or equal"
    				+ " than the number of total robots in this topology");
    		return;
    	}
    	
    	if(limited_angle >Math.PI)
    	{
    		System.out.println("the angle inaccuracy should be less than ¦Ð");
    		return;
    	}
    	//get @number different numbers between 0 and robots.size()-1
       int[] indexOfFaultyRobots = new int[number];
       ...
       //if current round < faultyround, the robot moves to a random destination which limited by a angle and a distance
       ...
       ((DefaultRobotScheduler)this.nodeScheduler).addPostComputeListener(new ClockListener() {
		...
		public void onClock() {
			...
			for(int j: indexOfFaultyRobots){
			     Robot faulty=robots.get(j);
				if(clocknumber < faulty.faultyRound){
					    double angle_error = Math.random()*limited_angle;
					    double distance_error = Math.random()*limited_distance;
					    System.out.println("robot " + faulty.name + ": angle imprecision is " + angle_error + "; distance imprecision is " + distance_error );
					    double angle = Math.atan2(((Destination2D)(faulty.dest)).dest.getY()-faulty.getLocation().getY(),
					    		((Destination2D)(faulty.dest)).dest.getX()-faulty.getLocation().getX());
						double faultyDest_x = faulty.getLocation().getX() + distance_error * Math.cos(angle+angle_error);
						double faultyDest_y = faulty.getLocation().getY() + distance_error * Math.sin(angle+angle_error);
						((Destination2D)(faulty.dest)).dest = new Point2D.Double(faultyDest_x,faultyDest_y);
				}		
			  }				
			}
